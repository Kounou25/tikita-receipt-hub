    ...
    return (
      <div className="min-h-screen bg-[#fafafa] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-lg font-medium text-gray-700">Chargement du reçu...</p>
        </div>
      </div>
    );
  }
  ...
  if (!receipt) {
    return (
      <div className="min-h-screen bg-[#fafafa] flex items-center justify-center px-4">
        <div className="bg-white border border-red-200 rounded-xl p-8 text-center shadow-lg max-w-md">
          <AlertCircle className="w-12 h-12 text-red-600 mx-auto mb-4" />
          <h3 className="text-xl font-bold text-black mb-2">Reçu introuvable</h3>
          <p className="text-gray-600 mb-6">Le reçu demandé n'existe pas ou a été supprimé.</p>
          <Link to="/receipts">
      ...
      <main className="pt-6 px-4 md:px-6 lg:px-8 pb-24 max-w-[1400px] mx-auto">
        ...
        {downloadProgress !== null && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[10000]">
            <div className="bg-white rounded-xl p-8 shadow-2xl flex flex-col items-center gap-6 max-w-sm w-full mx-4">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <div className="flex items-center gap-4 mb-2">
                <h1 className="text-3xl font-bold text-black">Reçu #{receipt.id}</h1>
                <Badge className={cn("px-4 py-1.5 text-sm font-medium rounded-full", getStatusBadge(receipt.status))}>
                  {receipt.status}
                </Badge>
              </div>
              <p className="text-lg text-gray-600">{receipt.date}</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          <div className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow">
            <h3 className="text-xl font-bold text-black mb-5">Informations du client</h3>
            <div className="space-y-4">
              <div>
                <p className="text-sm text-gray-600">Nom</p>
                <p className="text-lg font-semibold text-black">{receipt.client.name}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Téléphone</p>
                <p className="text-lg font-semibold text-black">{receipt.client.phone}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Email</p>
                <p className="text-lg font-semibold text-black">{receipt.client.email}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Adresse</p>
                <p className="text-lg font-semibold text-black">{receipt.client.address}</p>
              </div>
            </div>
          </div>

          <div className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow">
            <h3 className="text-xl font-bold text-black mb-5">Informations de l'entreprise</h3>
            <div className="space-y-4">
              <div>
                <p className="text-sm text-gray-600">Nom</p>
                <p className="text-lg font-semibold text-black">{receipt.company.name}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Téléphone</p>
                <p className="text-lg font-semibold text-black">{receipt.company.phone}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Email</p>
                <p className="text-lg font-semibold text-black">{receipt.company.email}</p>
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <p className="text-sm text-gray-600">NIF</p>
                  <p className="text-lg font-semibold text-black">{receipt.company.nif}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">RCCM</p>
                  <p className="text-lg font-semibold text-black">{receipt.company.rccm}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Articles */}
        <div className="bg-white border border-gray-200 rounded-xl hover:shadow-md transition-shadow mb-6 overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            <h3 className="text-xl font-bold text-black">Articles ({receipt.items.length})</h3>
          </div>
              ...
              <div key={index} className="p-6 hover:bg-gray-50 transition-colors">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <h4 className="text-lg font-semibold text-black">{item.designation}</h4>
                    <p className="text-gray-600 mt-1">
                      {item.quantity} × {item.unitPrice.toLocaleString('fr-FR')} FCFA
                    </p>
                  </div>
                  <p className="text-xl font-bold text-black ml-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow">
          <h3 className="text-xl font-bold text-black mb-6">Récapitulatif financier</h3>
          <div className="space-y-4 text-lg">
            <div className="flex justify-between">
              <span className="text-gray-600">Sous-total HT</span>
              <span className="font-semibold text-black">{receipt.subtotal.toLocaleString('fr-FR')} FCFA</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">TVA ({receipt.tva}%)</span>
              <span className="font-semibold text-black">{receipt.tvaAmount.toLocaleString('fr-FR')} FCFA</span>
            </div>
            {receipt.discount > 0 && (
            ...
            <div className="border-t-2 border-gray-200 pt-4">
              <div className="flex justify-between items-center">
                <span className="text-2xl font-bold text-black">Total TTC</span>
                <span className="text-3xl font-bold text-black">{receipt.total.toLocaleString('fr-FR')} FCFA</span>
              </div>
            </div>
